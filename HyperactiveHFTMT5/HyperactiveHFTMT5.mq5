...
input int      MomentumPeriod      = 30;       // Period for momentum calculation (ticks)
input double   BreakoutThreshold   = 0.0005;   // Minimum price movement for breakout (adjust per symbol)
input int      MinTickSpeed        = 5;        // Minimum ticks per second for entry
...
input double   PullbackPoints     = 2.5;        // Retrace points after breakout (2-3 points)
...
input int      VolatilityCycleSeconds = 4;      // Tick speed must be rising for N seconds (3-5)
...
int GetMomentumBreakoutSignal()
{
   ...
   // Reset breakout if too much time has passed (timeout) - shorter timeout for strategy tester
   if(breakoutDetected && (TimeCurrent() - breakoutTime) > 10)
   {
      breakoutDetected = false;
      breakoutPeakPrice = 0.0;
      breakoutDirection = 0;
