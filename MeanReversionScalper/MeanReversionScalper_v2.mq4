...

{
   ...
   int direction = EvaluateMeanReversionSignal(signalShift, activeDeviationMultiplier, activeMinDeviation);

   if(direction < 0 && UseQuickEntryBoost)
   {
...
int EvaluateMeanReversionSignal(const int shift, const double deviationMultiplier, const double minDeviation)

{

   if(deviationMultiplier <= 0.0) return -1;

   double effectiveMinDeviation = MathMax(0.1, minDeviation);

   double closePrice = iClose(Symbol(), EntryTimeframe, shift);

   if(closePrice <= 0) return -1;

   double ma = iMA(Symbol(), EntryTimeframe, MeanReversionPeriod, 0, MODE_SMA, PRICE_CLOSE, shift);
   ...
   double deviationPips = MathAbs(closePrice - ma) / point;
   ...
   if(closePrice < ma - (stdDev * deviationMultiplier))
      return OP_BUY;
...

{
   ...
   // Align MA/STD sample with the requested candle shift

   double ma = iMA(Symbol(), EntryTimeframe, period, 0, MODE_SMA, PRICE_CLOSE, shift);
