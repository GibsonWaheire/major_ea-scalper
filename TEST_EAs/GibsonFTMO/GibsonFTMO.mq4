...
input group "===== FTMO Compliance Settings ====="
enum FTMOLeverageOption
{
   Leverage_1_30 = 30,   // 1:30 Leverage (Max 3.33% risk per trade)
   Leverage_1_100 = 100  // 1:100 Leverage (Max 1% risk per trade)
};
input FTMOLeverageOption FTMOLeverage = Leverage_1_30;  // FTMO Account Leverage
input bool     EnforceFTMORules        = true;   // FTMO: Enforce strict 5% daily loss limit

input group "===== Risk Controls (Guardrails) ====="
input bool     UseGuardrails           = true;
input double   MaxDailyLossPercent        = 5.0;   // FTMO: Hard stop at 5% daily loss
...
{
   Print("========================================");
   Print("Gibson FTMO EA v1.00 Initialized");
   Print("========================================");
   Print("Strategy: FTMO-Compliant Scalping System");
   Print("Symbol: ", Symbol());
   ...
   if(EnforceFTMORules)
      Print("FTMO COMPLIANCE: ENABLED | Leverage: 1:", (int)FTMOLeverage, " | Hard Stop at ", MaxDailyLossPercent, "% daily loss");
   if(UsePendingOrders)
...
void OnDeinit(const int reason)
{
   ...
   Print("Gibson FTMO EA v1.00 Deinitialized. Reason: ", reason);
}
...
double CalculateDynamicLotSize()
{
   ...
   // FTMO: Calculate lot size based on leverage rules
   if(EnforceFTMORules && pipToPoint > 0.0 && StopLossPips > 0.0)
   {
      double maxRiskPercent = 0.0;
      if(FTMOLeverage == Leverage_1_30)
         maxRiskPercent = 3.33; // 1:30 = max 3.33% risk per trade
      else if(FTMOLeverage == Leverage_1_100)
         maxRiskPercent = 1.0;  // 1:100 = max 1% risk per trade
      
      if(maxRiskPercent > 0.0)
      {
         double maxRiskAmount = currentEquity * (maxRiskPercent / 100.0);
         double pipValuePerLot = GetPipValuePerLot();
         double slPips = StopLossPips;
         
         if(pipValuePerLot > 0.0 && slPips > 0.0)
         {
            double maxLotFromRisk = maxRiskAmount / (pipValuePerLot * slPips);
            double ftmoLot = MathMin(maxLotFromRisk, MaxLotSize);
            ftmoLot = MathMax(ftmoLot, MinLotSize);
            
            // Apply exposure scale
            ftmoLot = ftmoLot * effectiveLotScale;
            ftmoLot = MathMin(ftmoLot, MaxLotSize);
            ftmoLot = MathMax(ftmoLot, MinLotSize);
            
            return NormalizeDouble(ftmoLot, 2);
         }
      }
   }

   // Original lot sizing logic (fallback)
...
int OpenScalpTrade(int orderType)
{
   ...
   string comment = "GibsonFTMO " + (orderType == OP_BUY ? "BUY" : "SELL") + " Lot:" + DoubleToString(lotSize, 2);
...
void EvaluateGuardrails()
{
   ...
   if(shouldActivate && !guardrailActive)
   {
      ...
      CloseAllTrades("FTMO Guardrail triggered: Daily P&L " + DoubleToString(pnlPercent, 2) + "%");
      Print("FTMO GUARDRAIL TRIGGERED: Daily P&L ", DoubleToString(pnlPercent, 2), "% | LossHit=",
            (lossHit ? "true" : "false"), " | ProfitHit=", (profitHit ? "true" : "false"));
      Print("FTMO: Trading STOPPED for remainder of day. No new trades allowed until daily reset.");
      
      // FTMO: Hard stop - prevent all trading
      tradingAllowed = false;
   }
   // FTMO: Once guardrail is hit, it stays active until daily reset (no reset during same day)
   else if(EnforceFTMORules && guardrailActive && lossHit)
   {
      // Keep guardrail active - do not reset during same day
      tradingAllowed = false;
   }
   else if(!shouldActivate && guardrailActive && !EnforceFTMORules)
   {
      // Only reset if FTMO rules are not enforced
...
bool SubmitPendingOrder(int pendingType, double price)
{
   ...
   string comment = "GibsonFTMO " + side + " Lot:" + DoubleToString(lotSize, 2);
...
void CheckDailyReset()
{
   ...
   if(currentDay != lastDayReset)
   {
      ...
      tradingAllowed = true; // FTMO: Re-enable trading on daily reset
      CancelAllPendingOrders();
   }
}
...
void UpdateDisplay()
{
   ...
   string status = StringConcatenate(
      "==== Gibson FTMO EA v1.00 (FTMO-Compliant) ====\n",
