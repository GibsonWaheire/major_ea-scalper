//+------------------------------------------------------------------+
//|  HedgeScalper_v7.mq5  v7.00  — Directional Stack Scalper       |
//|                                                                   |
//|  PHASE 1 — STACK:                                               |
...
//+------------------------------------------------------------------+
#property copyright "HedgeScalper v7"
#property link      ""
#property version   "7.00"
#property description "Directional stack scalper — signal-driven transition hedge"
...
input group "=== Exit ==="
input double InpHarvestTarget = 5.0;  // Close all when basket net >= this ($)
...
bool OpenBuy(string lbl)
{
   ...
   bool   ok  = g_trade.Buy(NormLot(), _Symbol, ask, sl, 0, "HS7:"+lbl);
...
bool OpenSell(string lbl)
{
   ...
   bool   ok  = g_trade.Sell(NormLot(), _Symbol, bid, sl, 0, "HS7:"+lbl);
   if(ok) { g_lastOpen = TimeCurrent(); g_tradeEvents++; }
   return ok;
}
...
int OnInit()
{
   ...
   Print("HedgeScalper v7.00 — ", _Symbol,
         "  lot=", InpLot,
...
void OnTick()
{
   ...
   // ── TRANSITION NET EXIT ──────────────────────────────────────
   // In transition with both sides open: if net just turned positive → clean exit
   // Don't wait for full harvest target — any net positive clears the losing stack
   if(g_transition)
   {
      int buys  = CountSide(POSITION_TYPE_BUY);
      int sells = CountSide(POSITION_TYPE_SELL);
      if(buys > 0 && sells > 0 && floating >= 0)
      {
